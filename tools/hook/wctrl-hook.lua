--[[
    ...
    Starts the wctrl daemon when a mission begins, so the panels come to life
    without the user having to remember anything.

    Install to:  Saved Games/DCS/Scripts/Hooks/wctrl-hook.lua
...
-- WCTRL_DIR is substituted at install time so this hook points at the folder
...
local function logInfo(msg)
    pcall(function()
        dcsLog.write('WCTRL', dcsLog.INFO, msg)
    end)
end
...
function Wctrl.onSimulationStart()
    pcall(startDaemon)
end
